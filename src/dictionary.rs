...
pub(crate) const MAX_WORD_LENGTH: usize = 24;
...
pub(crate) const fn size_bits(word_length: usize) -> u8 {
    if word_length < SIZE_BITS_BY_LENGTH.len() {
        SIZE_BITS_BY_LENGTH[word_length]
    } else {
        0
    }
}

pub(crate) fn word(word_length: usize, word_index: usize) -> Option<&'static [u8]> {
    let bits = size_bits(word_length);
...
pub(crate) fn transform(
    ...
    let shift = size_bits(copy_length);
    if shift == 0 {
        return Err(DictionaryError::CopyLength);
    }
    ...
    let word = word(copy_length, word_index).ok_or(DictionaryError::Word)?;
...
#[cfg(test)]
mod tests {
    use super::{DictionaryError, size_bits, transform, word};
    ...
    #[test]
    fn exposes_dictionary_words_by_length_and_index() {
        assert_eq!(size_bits(4), 10);
        assert_eq!(word(4, 0), Some(b"time".as_slice()));
        assert_eq!(word(3, 0), None);
    }
